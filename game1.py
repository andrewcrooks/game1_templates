...
def remove_spaces(text):
    ...
    for x in range(0,len(text)):
        if text[x] != " ":
            ...
            for y in range(0,len(backwards_text)):
                if backwards_text[y] != " ":
                    if x == 0 and y == 0:
                        return(text)
                    else:
                        new_text = text[pointer:-y]
                        return(new_text)
    else:
        return("")
...
def print_menu(exits):
    ...
    for available_exits in exits:
        leads_to = exit_leads_to(exits,available_exits)
        print("Go " + available_exits + " to " + leads_to + "." )
...
def move(exits, direction):
    ...
    dictionary "exits" of avaiable exits, theypy choose to move towards the exit
